...
;// CONCATENATED MODULE: ./src/pages/api/questions.js

const cohere = __webpack_require__(8500);
cohere.init(process.env.COHERE_API_KEY);
function defaultPromt() {
    const topics = Object.keys(questions_namespaceObject);
    const random3Topics = topics.sort(()=>Math.random() - Math.random()).slice(0, 3);
    const randomQuestions = random3Topics.map((topic)=>{
        const randomQuestion = {
            ...questions_namespaceObject[topic][Math.floor(Math.random() * questions_namespaceObject[topic].length)],
            topic
        };
...
function handler(req, res) {
    ...
    if (!req.body.topics) return res.status(400).json({
